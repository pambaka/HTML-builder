...
async function buildPage() {
    ...
    if (!exists) {
      ...
      async function deleteFiles() {
        const entries = await fs.promises.readdir(
          path.join(__dirname, projectFolder),
        );
  ...
  copyFiles(
    ...
    path.join(__dirname, projectFolder, assetsFolder),
  );
}
...
function createHtmlFile() {
  ...
  const writeStream = fs.createWriteStream(
    path.join(__dirname, projectFolder, 'index.html'),
  );
    ...
    fs.readdir(path.join(__dirname, componenetsFolder), (err, entries) => {
      ...
      for (let i = 0; i < tags.length; i += 1) {
        ...
        const stream = fs.createReadStream(
          path.join(__dirname, componenetsFolder, tags[i]),
          { encoding: 'utf-8' },
        );
